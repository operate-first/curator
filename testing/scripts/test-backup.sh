...
mc -C /tmp/mc $MC_GLOBAL_FLAGS alias set \
	backup "$S3_ENDPOINT" "$AWS_ACCESS_KEY_ID" "$AWS_SECRET_ACCESS_KEY" >dev/null 2>&1
mc -C /tmp/mc $MC_GLOBAL_FLAGS find "backup/$BACKUP_DST"
[[ $? -ne 0 ]] && DIE "Fail to find BUCKET_DST=$BACKUP_DST. Check BACKUP_DST"

mc -C /tmp/mc $MC_GLOBAL_FLAGS find "backup/$BUCKET_NAME"
[[ $? -ne 0 ]] && DIE "Fail to find BUCKET_DST=$BUCKET_NAME. Check BUCKET_NAME"
[ -d "$BACKUP_SRC" ] || DIE "Directory $BACKUP_SRC does not exist."

echo "S3 verified with no error"
#mkdir upload
#mkdir download
